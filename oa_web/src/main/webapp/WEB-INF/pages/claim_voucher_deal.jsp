...
<%@page import="com.waziro.oa.global.Contant" %>
                    ...
                    <table id="message-table" class="table admin-form theme-warning tc-checkbox-1">
                        ...
                        <c:forEach items="${list}" var="cv">
                            <tr class="message-unread">
                                ...
                                <td class="hidden-xs">${cv.creater.name}</td>
                                <td class="fw600">${cv.totalAmount}</td>
                                <td><spring:eval expression="cv.createTime"/></td>
                                <td>
                                    <c:if test="${cv.status eq Contant.CLAIMVOUCHER_CREATED || cv.status eq Contant.CLAIMVOUCHER_BACK}">
                                        <a href="/claim_voucher/to_update?id=${cv.id}">修改</a>
                                        <a href="/claim_voucher/submit?id=${cv.id}">提交</a>
                                    </c:if>
                                    <c:if test="${cv.status eq Contant.CLAIMVOUCHER_SUBMIT || cv.status eq Contant.CLAIMVOUCHER_RECHECK}">
                                        <a href="/claim_voucher/to_check?id=${cv.id}">审核</a>
                                    </c:if>
                                    <c:if test="${cv.status eq Contant.CLAIMVOUCHER_APPROVED}">
